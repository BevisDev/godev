...
func TestDateTimeUTC_UnmarshalJSON_Valid(t *testing.T) {
	...
	expected, _ := time.Parse(layoutDateTimeUTC, "2024-04-21T15:30:00Z")
	if !d.Equal(expected) {
...
func TestDateTimeUTC_MarshalJSON(t *testing.T) {
	dt, _ := time.Parse(layoutDateTimeUTC, "2025-01-01T08:00:00Z")
...
func TestDateTimeUTC_Scan_String(t *testing.T) {
	...
	expected, _ := time.Parse(layoutDateTimeUTC, "2023-12-01T10:00:00Z")
	if !d.Equal(expected) {
		t.Errorf("Expected %v, got %v", expected, d.Time)
	}
}
