...
func NewLogger(cf *ConfigLogger) LoggerExec {
	var l = &AppLogger{
		profile:    cf.Profile,
		callerConf: cf.CallerConfig,
	}
	encoder := l.GetEncoderLog(cf)
	writer := l.WriteSync(cf)
...
func (l *AppLogger) GetEncoderLog(cf *ConfigLogger) zapcore.Encoder {
...
func (l *AppLogger) WriteSync(cf *ConfigLogger) zapcore.WriteSyncer {
	// handle profile dev
	if cf.Profile == "dev" {
...
func (l *AppLogger) LogRequest(req *RequestLogger) {
	...
	l.logger.WithOptions(
		zap.AddCallerSkip(l.callerConf.Request.Internal)).Info(
		"[===== REQUEST INFO =====]",
...
func (l *AppLogger) LogResponse(resp *ResponseLogger) {
	...
	l.logger.WithOptions(
		zap.AddCallerSkip(l.callerConf.Response.Internal)).Info(
		"[===== RESPONSE INFO =====]",
...
func (l *AppLogger) LogExtRequest(req *RequestLogger) {
	...
	l.logger.WithOptions(
		zap.AddCallerSkip(l.callerConf.Request.External)).Info(
...
func (l *AppLogger) LogExtResponse(resp *ResponseLogger) {
	l.logger.WithOptions(
		zap.AddCallerSkip(l.callerConf.Response.External)).Info(
