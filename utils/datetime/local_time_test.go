...
func TestDateTime_UnmarshalJSON_Valid(t *testing.T) {
	...
	expected, _ := time.Parse(DateTimeLayoutLocal, "2024-04-21T15:30:00")
	if !d.Equal(expected) {
...
func TestDateTime_MarshalJSON(t *testing.T) {
	dt, _ := time.Parse(DateTimeLayoutLocal, "2025-01-01T08:00:00")
	d := LocalTime{Time: dt}
...
func TestDateTime_Scan_String(t *testing.T) {
	...
	expected, _ := time.Parse(DateTimeLayoutLocal, "2023-12-01T10:00:00")
	if !d.Equal(expected) {
		t.Errorf("Expected %v, got %v", expected, d.Time)
	}
}
