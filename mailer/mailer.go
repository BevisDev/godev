...
func (m *smtpMailer) buildMessage(msg *Message) ([]byte, error) {
	...
	// write headers
	m.writeHeaders(&buf, msg, boundary)

	// write body
	if boundary == "" {
		buf.WriteString(msg.Body + consts.CRLF)
	} else {
		m.writeBodyPart(&buf, msg, boundary)
...
func (m *smtpMailer) writeHeaders(buf *bytes.Buffer, msg *Message, boundary string) {
	fmt.Fprintf(buf, "%s: %s%s", consts.From, m.cfg.From, consts.CRLF)
	fmt.Fprintf(buf, "%s: %s%s",
		consts.To, strings.Join(msg.To, ", "), consts.CRLF)
	if len(msg.Cc) > 0 {
		fmt.Fprintf(buf, "%s: %s%s",
			consts.Cc, strings.Join(msg.Cc, ", "), consts.CRLF)
	}

	subjectEncoded := base64.StdEncoding.EncodeToString([]byte(msg.Subject))
	fmt.Fprintf(buf, "%s: =?%s?B?%s?=%s",
		consts.Subject, consts.UTF8, subjectEncoded, consts.CRLF)
...
func (m *smtpMailer) writeBodyPart(buf *bytes.Buffer, msg *Message, boundary string) {
	fmt.Fprintf(buf, "--%s%s", boundary, consts.CRLF)
	contentType := consts.TextPlain
	if msg.IsHTML {
		contentType = consts.TextHTML
...
func (m *smtpMailer) writeAttachmentPart(buf *bytes.Buffer, att *Attachment, boundary string) {
	...
	for i := 0; i < len(encoded); i += lineLen {
		...
		fmt.Fprintf(buf, "%s%s", encoded[i:end], consts.CRLF)
		fmt.Fprintf(buf, "%s", consts.CRLF)
	}
}
