...
func NewRequest[T any](restClient *RestClient) HttpExec[T] {
	if restClient == nil {
		restClient = NewRestClient(new(HttpConfig))
	}
...
func (h *HttpClient[T]) restTemplate(c context.Context, method string) (*T, error) {
	...
	// log request
	var (
		isLog     = !validate.IsNilOrEmpty(h.rest.logger)
		startTime = time.Now()
	)
	if isLog {
	...
	ctx, cancel := utils.NewCtxTimeout(c, h.rest.timeout)
	defer cancel()
...
func (h *HttpClient[T]) requestInfoLogger(state, method, bodyStr string, startTime time.Time) {
	...
	h.rest.logger.LogExtRequest(reqLogger)
}
...
func (h *HttpClient[T]) execute(request *http.Request, startTime time.Time, state string) (*T, error) {
	...
	defer func() {
		...
		if isLog {
			...
			rest.logger.LogExtResponse(&respLogger)
		} else {
			sb.WriteString(fmt.Sprintf(consts.Body+": %s\n", bodyContent))
			sb.WriteString("==================================\n")
			log.Println(sb.String())
...
func (h *HttpClient[T]) skipLogBody(contentType string) bool {
	// check skip by api
	if h.skipLogBodyAPIs(h.url, h.rest.skipLogApis) {
		return true
	}

	// check skip content-type
	if !validate.IsNilOrEmpty(h.rest.skipContentType) && contentType != "" {
		for _, c := range h.rest.skipContentType {
